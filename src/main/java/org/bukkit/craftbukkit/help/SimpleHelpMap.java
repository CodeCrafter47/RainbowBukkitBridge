...
public class SimpleHelpMap implements HelpMap {
    ...
    private final FakeCraftServer server;
    private HelpYamlReader yaml;

    @SuppressWarnings("unchecked")
    public SimpleHelpMap(FakeCraftServer server) {
    ...
    public synchronized void initializeCommands() {
        ...
        outer: for (Command command : MyPlugin.commandMap.getCommands()) {
        ...
        for (Command command : MyPlugin.commandMap.getCommands()) {
            if (commandInIgnoredPlugin(command, ignoredPlugins)) {
                continue;
            }
            for (String alias : command.getAliases()) {
                // Only register if this command owns the alias
                if (MyPlugin.commandMap.getCommand(alias) == command) {
        ...
        fillPluginIndexes(pluginIndexes, MyPlugin.commandMap.getCommands());
